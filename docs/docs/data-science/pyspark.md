...
# Rename columns
project.select(
    ["project_id", "name_th", "area_total", "price_min", "price_max"]
).withColumnRenamed("name_th", "project_name").withColumnRenamed(
    "price_min", "project_price_min"
).withColumnRenamed(
    "price_max", "project_price_max"
)
...
df.withColumn("test", F.from_unixtime(df.create_date / 1000)).select(
    "test"
)  # epoch to timestamp
F.to_date("listing_update")  # timestamp to date
df.withColumn(
    "datetime_cst", F.from_utc_timestamp("datetime_utc", "CST")
).show()  # utc to tz
...
df.where(col("page") == "project").groupBy("fingerprint").agg(
    F.collect_list("eid2").alias("view_projects")
).select("fingerprint view_projects".split()).show()

# fillna with another column
a.withColumn("t", coalesce("field_property_project", "project_name"))
...
a.withColumn(
    "name_th", regexp_replace(trim(lower(col("name_th"))), string_clean_regex, "")
)
...
df2.withColumn("post_date_bp", F.col("post_date_bp")["$date"]).withColumn(
    "scrape_date", F.col("scrape_date")["$date"]
).select("post_date_bp", "scrape_date").show()

## by index
df2.withColumn("post_date_bp", F.col("post_date_bp").getItem(0)).withColumn(
    "scrape_date", F.col("scrape_date").getItem(0)
).show()
...
project.join(
    ...
    [
        "project_id"
    ],  # project.project_id == unittype.project_id in case keys are different, otherwise [COL_NAME] to prevent column duplicates
    how="left",
)

# Explode array
df = df.withColumn("tmp", F.explode("tmp")).select(
    *df.columns,
...
"""
partitionOverwriteMode_before = spark.conf.get(
...
base_path = write_retry_loop(
    df,
    partition_columns,
    write_mode,
    max_retry,
    wait_seconds_before_next_retry,
    override_base_path,
)
...
if partitionOverwriteMode_before != "dynamic":
    log.info(
        "Restore partitionOverwriteMode to '{}'".format(partitionOverwriteMode_before)
    )
    spark.conf.set(
        "spark.sql.sources.partitionOverwriteMode", partitionOverwriteMode_before
    )
```
...
df = (
    ...
    .option(
        ...
        "jdbc:oracle:thin:@//{}:{}/{}".format(
            os.environ["HOST"], os.environ["PORT"], os.environ["SID"]
        ),
    )
...
df.write.format("jdbc").option(
    "url", "jdbc:postgresql://{}:5432/{}".format(host, db_name)
...
### Override schema
```python
column_types = ", ".join(["{} VARCHAR(4000)".format(i) for i in string_cols])
a.option("createTableColumnTypes", column_types)
```
